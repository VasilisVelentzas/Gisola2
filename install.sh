...
echo "   ./gisolaBootstrap.py -c ../test/config.yaml --event-xml ../test/benchmark/event.xml"
echo ""
echo "============================================"
...
echo "      sudo apt-get install -y libnvvm4 cuda-cudart-12-9"
echo ""
echo " 3. Make the libraries available to numba on every conda activation:"
echo "      mkdir -p \$(conda info --base)/envs/gisola/etc/conda/activate.d"
echo "      cat > \$(conda info --base)/envs/gisola/etc/conda/activate.d/cuda.sh << 'EOF'"
echo "export CUDA_HOME=/usr/local/cuda-12.9"
echo "export LD_LIBRARY_PATH=/usr/local/cuda-12.9/targets/x86_64-linux/lib:\$LD_LIBRARY_PATH"
echo "EOF"
echo ""

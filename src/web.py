...
def renderEvent():
    ...
    # Determine inversion type (isotropic or deviatoric)
    keyinv_value = config.get_keyinv(config.cfg, org2) 
    if keyinv_value == 1: # Isotropic
        inversion_type = 'ISO'
        iso_display_value = '{:.2f}'.format(fm.moment_tensor.iso * 100)
    elif keyinv_value == 2: # Deviatoric
        inversion_type = 'DEV'
        iso_display_value = 'N/A'
    else:
        inversion_type = 'Unknown'
        iso_display_value = 'N/A'
...
def email():
    ...
    with smtplib.SMTP(config.cfg['Notification']['Email']['Smtp'].split(':')[0], config.cfg['Notification']['Email']['Smtp'].split(':')[1]) as server:
        ...
        for recipient in config.cfg['Notification']['Email']['Recipients'].replace(', ', ',').split(','):
            # Delete the "To" header if it exists from previous iteration
            if "To" in msg:
                del msg["To"]
            msg["To"] = recipient

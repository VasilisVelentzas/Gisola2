...
def compute_inversion_numbaCpu(G, obs_data, Dt, shifts_array, num_stations):
    ...
    VARDAT = np.sum(obs_data**2) / (3 * num_stations * 1024)
    for k in prange(n_sources):
...
def numba_warmup():
    ...
    # -----------------------------------------------------
    shift, mech, best_idx, vr, corr, singulars, condition = compute_inversion_numbaCpu(
        G_small,
        Dobs_small,
        0.1,
        Timeshifts_small,
    ...
    print(f"Warmup time elapsed: {end - start:.3f} sec")
...
def run_python_inversion(setup, job):
    ...
    endLoad = time.time()
    print('Files Loaded. Time elapsed: ', endLoad - startInv)
    ...
    endElemse = time.time()
    print("Elementary seismograms calculated. Time elapsed: ", endElemse - startElemse)
    ...
    shift, mech, best_idx, vr, corr, singulars, condition = \
        compute_inversion_numbaCpu(G, Dobs, Dt, Timeshifts, nr)
    kernelTime = time.time() - startKernel
    print('Inversions Computed. Kernel time elapsed: ', kernelTime)
...
def _hudson_cloud_plot(all_mt_objects, VR_all, center_mt, label, outpath):
    """Hudson projection of the bootstrap cloud with one highlighted solution."""
    u_vals, v_vals = [], []
    for mt in all_mt_objects:
    ...
    norm = mcolors.Normalize(vmin=VR_all.min(), vmax=VR_all.max())
    cmap = plt.get_cmap('viridis')
    colors = cmap(norm(VR_all))

    fig, ax = plt.subplots(figsize=(8, 8))
    hudson.draw_axes(ax)

    sort_idx = np.argsort(VR_all)  # higher VR plotted on top
    ax.scatter(u_vals[sort_idx], v_vals[sort_idx], c=colors[sort_idx],
...
def getBootstrapping(num_iterations=100, num_realizations=100):
    ...
    for weights in dirichlet_samples:
        ...
        W.append(W_single)
    W = np.array(W)
    print("Bootstrap: W matrix", W.shape,
          ", time elapsed:", round(time.time() - startBoot, 2), "s")

    # =========================================================
    # 2. Grid coordinates from the source files
    # =========================================================
    files = sorted(
        glob.glob(os.path.join(config.workdir, 'sources', 'grid0', '*.dat')),
        key=lambda f: int(''.join(c for c in os.path.basename(f) if c.isdigit())))
    grid = np.vstack([np.loadtxt(f, usecols=(0, 1, 2, 3), ndmin=2) for f in files])

    crow = grid[grid[:, 0] == best_idx + 1][0]
    Xcmt, Ycmt, Zcmt = crow[1], crow[2], crow[3]
    ...
    print("Max VR:", vr.max(), ", thresh 80%:", ErrorTresh)
    print("Grid boundaries: X [", xmin, ",", xmax, "] Y [", ymin, ",", ymax,
          "] Z [", zmin, ",", zmax, "]")
    print("CMT coordinates: X:", Xcmt, ", Y:", Ycmt, ", Z:", Zcmt)
    print("Bootstrap box:   X [", xminb, ",", xmaxb, "] Y [", yminb, ",", ymaxb,
          "] Z [", zminb, ",", zmaxb, "] T [", tminb, ",", tmaxb, "]")
    ...
    try:
        ...
        print("Grid percentage:", Grid_Percentage, ", points:", len(idx), "/",
              grid.shape[0], ", true percentage:", len(idx) / grid.shape[0])
    except ZeroDivisionError:
        pass
    ...
    print("Bootstrap GF selection time elapsed:",
          round(time.time() - startBoot, 2), "s ->", Gboot.shape[0],
          "grid points,", len(TimeshiftsBoot), "shifts")
    ...
    try:
        ...
        plt.close(fig)
        print("Grid/VR overview figure saved:",
              os.path.join(plotdir, 'grid_vr_bootstrap.png'))
    except Exception:
        import traceback
        traceback.print_exc()
        print("WARNING: grid/VR figure failed (bootstrap continues)")

    # =========================================================
    # 3. Warm the bootstrap kernel (so iteration timings are clean)
    # =========================================================
    t0 = time.time()
    compute_bootstraping_numbaCpu(Gboot[:2], Dobs, W[0, :], Dt,
                                  np.array([0], dtype=np.int64), Coords[:2])
    print("Bootstrap kernel warmup:", round(time.time() - t0, 2), "s")
    ...
    print("Bootstrap iterations done:", num_iterations, "x", Gboot.shape[0],
          "grid points =", round(itersTime, 2), "s",
          "({} ms / iteration)".format(round(1000.0 * itersTime / num_iterations)))
    ...
    print("Bootstrap fit boundaries, Max:", VR_all.max(), ", Min:", VR_all.min())
    ...
    # =========================================================
    endBoot = time.time()
    p1, p2 = result["BestPlane1"], result["BestPlane2"]
    print("\n" + "=" * 64)
    print(" FINAL SUMMARY")
    print("=" * 64)
    print(" Inversion : load {:.2f} s, elemse {:.2f} s, kernel {:.2f} s, total {:.2f} s".format(
        invtimes['load'], invtimes['elemse'], invtimes['kernel'], invtimes['total']))
    computeTime = (endBoot - startBoot) - plotsTime
    print(" Bootstrap : {} iterations x {} grid points = {:.2f} s "
          "(compute total {:.2f} s, figures excluded)".format(
        num_iterations, Gboot.shape[0], itersTime, computeTime))
    print(" Best VR   : {:.2f} % (shift {}, chunk {}, grid point {})".format(
        result['VR'], result['BestShift'], result['BestChunk'], best_idx + 1))
    print(" Plane 1   : strike {:.1f}, dip {:.1f}, rake {:.1f}".format(*p1))
    print(" Plane 2   : strike {:.1f}, dip {:.1f}, rake {:.1f}".format(*p2))
    print(" Mw {:.2f}, M0 {:.3e}, DC {:.1f} %, CLVD {:.1f} %, ISO {:.1f} %".format(
        result['BestMw'], result['BestMo'], result['BestDC'],
        result['BestCLVD'], result['BestISO']))
    print(" Boot mean : plane1", np.round(plane1_mean, 1),
          ", plane2", np.round(plane2_mean, 1))
    print("             Mw {:.2f}, DC {:.1f} %, CLVD {:.1f} %, ISO {:.1f} %".format(
        MagnitudesMean[1], SolutionPartsMean[0], SolutionPartsMean[1],
        SolutionPartsMean[2]))
    print(" MT mean   :", mt_mean)
    print(" MT std    :", mt_std)
    print(" Figures   : {:.2f} s (independent of compute) -> {}".format(plotsTime, plotdir))
    print("=" * 64)
...
def _make_bootstrap_figures(plotdir, Dobs, result, VR_all, indices,
    ...
    # ---- moment tensor objects (filtered like TimeTest2) ----
    all_mt_objects = []
    for res in BootstrapAngles:
        for mmat in res[-1]:
            all_mt_objects.append(pmt.MomentTensor(m=mmat))
    all_mt_objects = [all_mt_objects[i] for i in indices]
    ...
    _hudson_cloud_plot(all_mt_objects, VR_all, best_mt, 'Best Fit',
                       os.path.join(plotdir, 'hudson_best.png'))
    _hudson_cloud_plot(all_mt_objects, VR_all, mean_mt, 'Mean solution',
                       os.path.join(plotdir, 'hudson_mean.png'))
    _hudson_cloud_plot(all_mt_objects, VR_all, median_mt, 'Median solution',

...
# local libs
import event, inventory, stream, isola2, plot, web, config
import time
import warnings
warnings.filterwarnings('ignore')

def _stage(name, t0, store):
    dt = time.time() - t0
    store[name] = store.get(name, 0.0) + dt
    print('[STAGE] {:<34s}: {:7.2f} s'.format(name, dt), flush=True)
...
_compute_device = str(_cfg.get('ComputeDevice', 'CPU')).upper()
if _compute_device == 'GPU':
    try:
        from numba import cuda as _cuda
        _gpu_ok = len(list(_cuda.gpus)) > 0
    except Exception:
        _gpu_ok = False
    if _gpu_ok:
    ...
    print('[INFO] Compute device : CPU', flush=True)
# ----------------------------------------------------------------------
...
if not args.revise:
    ...
    for evt in cat:
        ...
        try:
            ...
            _stage('read inventory', _t, _STAGE_T)
            ...
            if not args.station:
                ...
                config.logger.info(config.inv)
                _stage('station selection (distance)', _t, _STAGE_T)
          
            _t = time.time()
            stream.getWaveforms()
            _stage('read waveforms', _t, _STAGE_T)

            _t = time.time()
            stream.clean()
            _stage('quality checks (snr/clip/mouse) + correction', _t, _STAGE_T)

            _t = time.time()
            inventory.selectAzm()
            _stage('azimuthal selection', _t, _STAGE_T)
            ...
            _t = time.time()
            stream.prioritize()
            _stage('prioritize + save corrected', _t, _STAGE_T)
            _STAGE_T['preprocessing TOTAL (read->axitra)'] = time.time() - _tpre
            print('[STAGE] {:<34s}: {:7.2f} s'.format('preprocessing TOTAL (read->axitra)',
                  _STAGE_T['preprocessing TOTAL (read->axitra)']), flush=True)
           
            BootstrapFlag = 1
        
            # Stack them one after another along the first axis
            _t = time.time()
            isola2.getGreens()              # sources/crustals/grdat + AXITRA
            _stage('green function calculation (axitra)', _t, _STAGE_T)

            _t = time.time()
            isola2.getInversions()
            _stage('full inversion', _t, _STAGE_T)

            _t = time.time()
            isola2.exportResults()
            _stage('export results', _t, _STAGE_T)
            if(BootstrapFlag==1):
                _t = time.time()
            ...
            _t = time.time()
            isola2.renderSite()
            _stage('site (figures + html)', _t, _STAGE_T)

            # ---- Final summary (after all stages) ----
            _s = isola2._SUMMARY
            if _s is not None:
                _p1, _p2 = _s['result']['BestPlane1'], _s['result']['BestPlane2']
                print('\n' + '=' * 64)
                print(' FINAL SUMMARY')
                print('=' * 64)
                print(' Inversion : load {:.2f} s, elemse {:.2f} s, kernel {:.2f} s, total {:.2f} s'.format(
                    _s['invtimes']['load'], _s['invtimes']['elemse'],
                    _s['invtimes']['kernel'], _s['invtimes']['total']))
                print(' Bootstrap : {} iterations x {} grid points = {:.2f} s'
                      ' (compute total {:.2f} s, figures excluded)'.format(
                    _s['num_iterations'], _s['n_grid'],
                    _s['itersTime'], _s['computeTime']))
                print(' Best VR   : {:.2f} % (shift {}, chunk {}, grid point {})'.format(
                    _s['result']['VR'], _s['result']['BestShift'],
                    _s['result']['BestChunk'], _s['best_idx'] + 1))
                print(' Plane 1   : strike {:.1f}, dip {:.1f}, rake {:.1f}'.format(*_p1))
                print(' Plane 2   : strike {:.1f}, dip {:.1f}, rake {:.1f}'.format(*_p2))
                print(' Mw {:.2f}, M0 {:.3e}, DC {:.1f} %, CLVD {:.1f} %, ISO {:.1f} %'.format(
                    _s['result']['BestMw'], _s['result']['BestMo'],
                    _s['result']['BestDC'], _s['result']['BestCLVD'],
                    _s['result']['BestISO']))
                print(' Boot mean : plane1', np.round(_s['plane1_mean'], 1),
                      ', plane2', np.round(_s['plane2_mean'], 1))
                print('             Mw {:.2f}, DC {:.1f} %, CLVD {:.1f} %, ISO {:.1f} %'.format(
                    _s['MagnitudesMean'][1], _s['SolutionPartsMean'][0],
                    _s['SolutionPartsMean'][1], _s['SolutionPartsMean'][2]))
                print(' MT mean   :', _s['mt_mean'])
                print(' MT std    :', _s['mt_std'])
                print(' Figures   : {:.2f} s (independent of compute) -> {}'.format(
                    _s['plotsTime'], _s['plotdir']))
                print('=' * 64)

            print('\n' + '='*62)
            print(' PIPELINE TIMING SUMMARY')
            print('='*62)
            for _k, _v in _STAGE_T.items():
                print(' {:<28}: {:8.2f} s'.format(_k, _v))
            print(' {:<28}: {:8.2f} s'.format('TOTAL', sum(_STAGE_T.values())))
            print('='*62)
            #isola2d.gatherResults()
            #plot.allplots()
...
# if revise flag is enabled
    ...
    for eventid in args.event:
        try:
            ...
            isola.calculateRevisedInversions(_cfg,logger,workdir,bestinvdir, evt_obj=evt)
